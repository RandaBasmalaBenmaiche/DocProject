...
const Cours = ({ modules, data, onChange }) => {
    ...
    const coursCall = async () => {
        const config = {
            headers: {
                'authorization': `Bearer ${token.token}`
            }
        }
        axios.get(`${API_BASE}/coure/couremodule/` + modules, config).then(res => {
            setCours(res.data)
            setLoading(false)
        }).catch(error => {
    ...
    return (
        ...
        <MenuList sx={{ width: '100%', borderRadius: 2, p: 1 / 2, mb: 2, mt: 1 }}>
            ...
            <Grid container xs={12} sx={{ display: 'flex', p: 1, backgroundColor: 'white', borderRadius: 2, boxShadow: '3px 3px 6px rgba(0,0,0,0.2)' }}>
                {check.length > 0 && modules ? check.map(check => {
                    return (
                        <Grid item xs={4}
                            sx={{
                                display: 'flex',
                    ...
                    :
                    <p>try to select module or another module</p>
                }
            </Grid>
        </MenuList>
