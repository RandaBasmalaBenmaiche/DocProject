...
const TypeQ = ({ onChange }) => {
    ...
    const handleqrocChecked = (event) => {
        ...
        onChange({ qcm: qcmChecked, qcs: qcsChecked, croc: event.target.checked, clinique: casqliniqueChecked });
    };

    const handlecasqliniqueChange = (event) => {
        setcasqliniqueChecked(event.target.checked);
        onChange({ qcm: qcmChecked, qcs: qcsChecked, croc: qrocChecked, clinique: event.target.checked });
    };

    const handleQcsChange = (event) => {
        setQcsChecked(event.target.checked);
        onChange({ qcm: qcmChecked, qcs: event.target.checked, croc: qrocChecked, clinique: casqliniqueChecked });
    };

    const handleQcmChange = (event) => {
        setQcmChecked(event.target.checked);
        onChange({ qcm: event.target.checked, qcs: qcsChecked, croc: qrocChecked, clinique: casqliniqueChecked });
    };
    return (
