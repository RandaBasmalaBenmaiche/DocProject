...
import { getModule, reset as resetmodule } from "../../../features/modules/modulesSlice";
...
const InfoModule = () => {
    ...
    const history = (id,name,Id) => {
        ...
        if (localStorage.getItem('cours')) {
            ...
            newdata.push({id : id, module:Id, name: name, date : today})
            console.log(newdata)
            localStorage.setItem('cours',JSON.stringify(newdata))
        } else {
            localStorage.setItem('cours',JSON.stringify([{id : id, module:Id, name: name, date : today}]))
        }
    }
    const WatingCard = () => {
        return (
        ...
        return (
            <Grid container item xs={12} sm={5.9} md={3.9}
                ...
                <Grid item xs={4} sx={{ p: 1, display: 'flex', alignItems: 'flex-start', justifyContent: 'center' }}>
                    <img
                        style={{ borderRadius: '50%',width:'100%',maxHeight:'80%', }}
                        // height={80}
                        // width={100}
                ...
                <div className="icons"
                    ...
                    <Link to={"/simple/cours/" + cours._id +"/"+ params.id} className='Links'>
                        <IconButton onClick={()=>history(cours._id, cours.name, params.id)} sx={{ textAlign: 'center' }} >
            ...
            <Grid container spacing={2} sx={{ display: 'flex', p: 2, paddingBlock: 4 }}>
                ...
                <Grid container item md={12} xs={12}   >
                    ...
                    <Grid item md={2} sm={4} xs={6}>
                        <Link to={"/simple/QuizeC/"+module._id} className='Links'>
                            <IconButton
