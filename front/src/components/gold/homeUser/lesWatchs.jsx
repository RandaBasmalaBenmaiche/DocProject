...
import image from './image/videWatch.png';



const LestWatchs = ({ data, Delete }) => {
    const [vide, setVide] = React.useState(false);
    const video = data.filter((item, pos) => { return data.indexOf(item) == pos}).slice(-5).reverse() 
    const testVide=()=>{
        if(data[0] == undefined){
            setVide(true);
        }else{
                    ...
                    video.map(video => {
                        return (
                            <Grid item xs={5.5} sm={2.6} md={2.6}
                                ...
                                <Typography variant='body1' sx={{  maxHeight: 45, fontFamily: 'Bahnschrift SemiBold',  }} >
                                    {video.name}
                                </Typography>
                                <Grid className="option" item xs={12}
                                    ...
                                    <div style={{ flex: '1 0 50%', borderRight: '1px solid rgba(0,0,0,0.15)', }}>
                                        ...
                                        <IconButton aria-label="delete" onClick={() => { Delete(video._id) }}>
                                            <DeleteForeverIcon sx={{ color: '#F55050' }} />
                                        </IconButton>

                                    </div>
                                    <div style={{ flex: '1 0 50%' }}>
                                        <Link to={`/gold/cours/${video.coure}/${video.module}`}>
