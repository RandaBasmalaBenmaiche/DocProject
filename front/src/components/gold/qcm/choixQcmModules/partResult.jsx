...
console.log(filtred)
    ...
    const saveData = async () => {
        ...
        await axios.post(`${API_BASE}/quiz`, filtred,config).then(res => location.push(`/gold/consulteQuize/${res.data._id}`)).catch(error => console.log(error.message))
    }
    return (
        <Grid container item md={4} xs={12} sx={{ p: 0, mt: 3, }}  >
            <Grid container xs={12} sx={{ maxHeight: 130, boxShadow: '2px 2px 4px rgba(0,0,0,0.2)', display: 'block', p: 1, mb: 1, backgroundColor: '#635985', borderRadius: 2 }}>
                <Grid xs={12} sx={{ display: 'flex', alignItems: 'center', paddingInline: 1, paddingBlock: 1, }}>
                    <Typography variant="h6" sx={{fontFamily: 'Bahnschrift SemiBold', p: 0, color: 'white', flex: '1 0 85%', }}>
                        Question
                    </Typography>
                ...
                <Grid container xs={12} sx={{ display: 'block', backgroundColor: 'white', borderRadius: 2, overflow: 'hidden', mt: 2,p:2,boxShadow:'3px 3px 6px rgba(0,0,0,0.2)','&:hover': { backgroundColor: '#F5F5F5' } }}>
                    <Grid xs={12} sx={{ display: 'flex', alignItems: 'center', paddingInline: 1, paddingBlock: 1 / 2,}}>
                        <Typography variant="body1" sx={{p:1,fontFamily: 'Bahnschrift SemiBold',}}>
                            {loading ?
